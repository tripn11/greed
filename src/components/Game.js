...
const Game = () =>{
    ...
    const [computerRolls, setComputerRolls] = useState(0)
    ...
    return(
        <div>
            <p>Target:{target}</p>
            <Player player={1} target = {target} details={user1}/>
            <Player player={opponent === 'friend' ? 2:0} target = {target} details ={user2}/>
            <img src={image}/>
...
//             let computerRolls = 0;
//             setButtonDisabled(true)
//             const expected = Math.ceil(2 + (Math.random() * 3))
//             const computerRollInterval = setInterval(() => {
//                 if (activeUser || user1.bank >= target || user2.bank >= target) {
//                     clearInterval(computerRollInterval);
//                     return;
